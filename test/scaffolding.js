...
var createGridHtml = function(settings) {
    ...
    var html = '<table ui-scroll-viewport ' + viewportStyle + ' >' +
                '<thead style="display:block">' +
                    '<tr>';
    columns.forEach((col) => { html += '<th ui-scroll-th >' + col + '</th>'; });
        html +=     '</tr>' +
                '</thead>' +
                '<tbody>' +
                    '<tr ui-scroll="item in ' + settings.datasource + '"' + adapter + '>';
    columns.forEach((col) => {html + '<td ui-scroll-td >{{item.' + col + '}}</td>'});
        html +=     '</tr>' +
                '</tbody>' +
            '</table>';
    return html;
